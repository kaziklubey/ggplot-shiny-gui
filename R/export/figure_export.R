...
# Validate an export after its writer/device has released ownership. Figure PNG
# writes directly to Shiny's download target, matching the proven Graph PNG path;
# this helper intentionally does no second copy/rename operation.
figure_validate_written_export <- function(path, label = "Figure export") {
  path <- as.character(path %||% "")[1]
  label <- as.character(label %||% "Figure export")[1]
  if (!nzchar(path) || !file.exists(path)) {
    stop(paste0(label, "ファイルを生成できませんでした。"))
  }
  sz <- suppressWarnings(file.info(path)$size)
  if (!length(sz) || !is.finite(sz[[1]]) || sz[[1]] <= 0) {
    stop(paste0(label, "ファイルが空です。"))
  }
  invisible(TRUE)
}
