# v3.70.0: extracted from server.R; sourced into the same server function environment.
  ...
  output$graph_tab_bar <- renderUI({
    ...
    tagList(lapply(seq_len(nrow(meta)), function(i) {
      ...
      tags$button(
        ...
        `data-graph-id` = id,
        onclick = sprintf(
          "if (window.ggplotGuiSelectGraphLive) return window.ggplotGuiSelectGraphLive('%s'); if (window.Shiny) Shiny.setInputValue('graph_client_selected', '%s', {priority:'event'}); return false;",
          id, id
        ),
        ondblclick = sprintf(
        ...
        title = "クリック: このGraphを表示・編集 / ダブルクリック: 名前変更",
        nm
      )
    }))
  })
  ...
  # it later to a shifted row/column index.
  figure_apply_rapid_layout_edits <- function(st, edits, valid_graph_ids, valid_asset_ids) {
    if (!is.list(st) || !is.list(edits) || !length(edits)) {
      return(list(state = st, changed = FALSE, count = 0L))
    }
    changed <- FALSE
    count <- 0L
    for (edit in edits) {
      if (!is.list(edit)) next
      typ <- as.character(edit$type %||% "")[1]
      if (!typ %in% c("row_height", "column_ratio", "graph_width", "graph_height")) next
    ...
    list(state = st, changed = changed, count = count)
  }
  ...
  observeEvent(input$figure_layout_edit, {
    ...
    queued <- if (identical(typ0, "rapid_batch")) e$edits else e$pendingEdits
    rapid <- figure_apply_rapid_layout_edits(st, queued, valid_graph_ids, valid_asset_ids)
    if (isTRUE(rapid$changed)) {
      st <- rapid$state
      figure_layout_state(st)
      diag_log("FIGURE-EDIT", paste0("rapid batch count=", rapid$count, " changed=TRUE"))
    }
  ...
  # value. Mode/checkbox/source changes remain immediate.
  figure_rapid_input_debounce_ms <- suppressWarnings(as.integer(get0(
    "FIGURE_RAPID_INPUT_DEBOUNCE_MS", ifnotfound = 180L, inherits = TRUE
  )))
  if (!is.finite(figure_rapid_input_debounce_ms) || figure_rapid_input_debounce_ms < 50L) {
    figure_rapid_input_debounce_ms <- 180L
  }

  observeEvent(
    list(
      input$figure_label_mode, input$figure_label_anchor,
      input$figure_legend_override, input$figure_legend_title,
      input$figure_legend_background, input$figure_align_h, input$figure_align_v,
      input$figure_crop_enabled, input$figure_inset_enabled, input$figure_inset_source,
      input$figure_inset_border, input$figure_external_legend_mode,
      input$figure_external_legend_position
    ),
    { capture_current_figure_override() },
    ignoreInit = TRUE
  )

  figure_rapid_override_signal <- reactive({
    list(
      input$figure_panel_label, input$figure_panel_label_size, input$figure_top_gutter,
      input$figure_label_x_offset, input$figure_label_y_offset,
      input$figure_label_x, input$figure_label_y, input$figure_legend_gap,
      input$figure_crop_left, input$figure_crop_right, input$figure_crop_top, input$figure_crop_bottom,
  ...
  figure_rapid_override_commit <- shiny::debounce(
    figure_rapid_override_signal, millis = figure_rapid_input_debounce_ms
  )
  observeEvent(figure_rapid_override_commit(), {
    capture_current_figure_override()
  }, ignoreInit = TRUE)

  # F1-4e: legend X/Y retain field-specific ownership, but rapid spinner steps
  # are coalesced before one canonical Figure override commit.
  figure_legend_x_commit <- shiny::debounce(reactive(input$figure_legend_x), millis = figure_rapid_input_debounce_ms)
  figure_legend_y_commit <- shiny::debounce(reactive(input$figure_legend_y), millis = figure_rapid_input_debounce_ms)
  observeEvent(figure_legend_x_commit(), { capture_current_figure_override("x") }, ignoreInit = TRUE)
  observeEvent(figure_legend_y_commit(), { capture_current_figure_override("y") }, ignoreInit = TRUE)

  # F1-5: numeric Inset geometry edits likewise commit once after a short idle.
  figure_inset_x_commit <- shiny::debounce(reactive(input$figure_inset_x), millis = figure_rapid_input_debounce_ms)
  figure_inset_y_commit <- shiny::debounce(reactive(input$figure_inset_y), millis = figure_rapid_input_debounce_ms)
  figure_inset_width_commit <- shiny::debounce(reactive(input$figure_inset_width), millis = figure_rapid_input_debounce_ms)
  figure_inset_height_commit <- shiny::debounce(reactive(input$figure_inset_height), millis = figure_rapid_input_debounce_ms)
  observeEvent(figure_inset_x_commit(), { capture_current_figure_override("none", "x") }, ignoreInit = TRUE)
  observeEvent(figure_inset_y_commit(), { capture_current_figure_override("none", "y") }, ignoreInit = TRUE)
  ...
  observeEvent(input$figure_panel_clicked, {
    ...
    # If the user clicks another Panel inside the rapid-input quiet window,
    # persist the currently visible Inspector values for the old owner first.
    # capture_current_figure_override() is generation/id gated, so programmatic
    # Inspector hydration remains excluded.
    ...
    if (nzchar(id)) {
      ...
      if (id %in% names(isolate(figure_external_assets()))) {
        ...
        if (isTRUE(has_edit) && isTRUE(editor_active) && !identical(editing_id, id)) {
          ...
          diag_log(
            "FIGURE-EDITOR-SHELL",
            paste0("selection-follow previous=", editing_id, " accepted=", switched),
            id = id
          )
        }
      }
