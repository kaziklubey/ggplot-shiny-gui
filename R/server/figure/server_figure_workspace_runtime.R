# v3.70.0: extracted from server.R; sourced into the same server function environment.
  ...
  write_figure_export <- function(path, format) {
    ...
    if (isTRUE(raster_png_mode)) {
      # Graph PNG export already writes directly to downloadHandler's content
      # target on Windows. Figure PNG uses the same single-owner lifecycle:
      # open the target once, draw, close, then validate the completed file.
      # Avoid a second temp->target copy/rename phase, which can race Shiny's
      # download target lifecycle on Windows.
      app_open_png_device(
    ...
    if (isTRUE(raster_png_mode)) {
      grDevices::dev.off()
      closed <- TRUE
      figure_validate_written_export(path, label = "Figure PNG")
      diag_log("FIGURE-PNG-COMMIT", paste0("direct_target=TRUE bytes=", file.info(path)$size %||% NA))
    }
    invisible(TRUE)
  }
