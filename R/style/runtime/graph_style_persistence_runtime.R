...
# import/export/copy/paste behavior.
  ...
  style_settings <- reactive({
    cfg <- list(
      ...
      fill_none_styles = fill_none_styles(),
  ...
  apply_style_config <- function(cfg, success_message = "書式を適用しました。", release_guard = TRUE,
    ...
    fill_none_styles(graph_bar_box_normalize_fill_none_tree(cfg$fill_none_styles %||% list()))
    ...
    if (!is.null(a)) {
      ...
      updateNumericInput(
        session, "bar_border_dash",
        value = graph_bar_box_clamp_dash_unit(a$bar_border_dash %||% 4, 4L)
      )
      updateNumericInput(
        session, "bar_border_gap",
        value = graph_bar_box_clamp_dash_unit(a$bar_border_gap %||% 2, 2L)
      )
